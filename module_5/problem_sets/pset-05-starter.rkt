...
(define (render-game game)
  (place-image PADDLE
               (game-paddle game) PADDLE-CTR-Y
               (render-balls (game-balls game))))
...
(check-expect (game-with-next-balls G1)
              (make-game (next-balls (cons
                                      (make-ball (/ WIDTH 2) (/ HEIGHT 2) 4 -3)
                                      empty)) (/ WIDTH 2)))
...
(check-expect (catch-balls empty 100) empty)
(check-expect (catch-balls (cons B1 (cons
                                     (make-ball 50 PADDLE-CTR-Y 3 1)empty)) 50)
              (cons B1 empty))
...
(check-random (handle-mouse (make-game empty 5) 100 100 "button-up")
              (make-game empty 5))
...
(check-expect (handle-key (make-game empty 50) "left")
              (make-game empty (- 50 10)))


(check-expect (handle-key (make-game empty 80) "left")
              (make-game empty (- 80 10)))


(check-expect (handle-key (make-game empty 80) "right")
              (make-game empty (+ 80 10)))
