...
; (define (pyramid n i) empty-image) ; stu
...
(define (pyramid n img)
  (cond [(zero? n) empty-image]
        [else
         (above   ; n is added because it's often useful                   
              (pyramid (sub1 n) img)
              (build-layer n img))]))
...
(check-expect (build-layer 1 COOKIES) COOKIES)
(check-expect (build-layer 2 COOKIES) (beside COOKIES COOKIES))
(check-expect (build-layer 3 COOKIES) (beside COOKIES COOKIES COOKIES))

(@template Natural)

(define (build-layer n img)
  (cond [(= n 1) img]
        [else
         (beside img (build-layer (- n 1) img))]))
...
(define (bubble? b)
  (cond [(string=? b "solid") false]
        [(string=? b "bubble") true]))
...
(define (sink lob)
  (cond [(empty? (rest lob)) empty]
        [else 
         (if (and (solid? (first lob)) (bubble? (first (rest lob))))
             (cons (first (rest lob))
                   (sink (cons (first lob) (rest (rest lob)))))
