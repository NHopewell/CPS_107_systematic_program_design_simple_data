...
;;Given two images, return the image that has the greatest height (image-height)
;; NOTE: if the images are the same height, return first image

(define SRECT (rectangle 20 10 "solid" "black"))
(define TRECT (rectangle 20 30 "solid" "black"))
(define SQUARE (square 20 "solid" "black"))

(check-expect (taller-image SRECT TRECT) TRECT)
(check-expect (taller-image SQUARE SRECT) SQUARE)
(check-expect (taller-image SRECT SRECT) SRECT)

;; (define (taller-image i1 i2) SQUARE)    ;; this is the stub
