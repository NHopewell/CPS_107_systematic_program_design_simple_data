...
(@htdf lookup)
(@signature BST Integer -> String)


(check-expect (lookup BST0 10) false)
(check-expect (lookup BST42 500) false)
(check-expect (lookup BST3 3) "ilk")
(check-expect (lookup BST42 50) "dug")
  ...
  (cond [(false? tree) false]
        [else
         (cond[(= (node-key tree) key)
                   (node-val tree)]
              [(< key (node-key tree))
                  (lookup (node-l tree) key)]
              [(> key (node-key tree))
                  (lookup (node-r tree) key)])]))
