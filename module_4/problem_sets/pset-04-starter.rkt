...
(define B2 (make-ball (/ WIDTH 2) (/ HEIGHT 2) 2 2))
...
(define LOB3 (cons B1 (cons B1 empty)))
...
(define (main b)
  (big-bang b
            (on-draw   render-balls)   ;Ball -> Image
            (on-tick   next-ball)     ;Ball -> Ball
            (on-key    handle-key)   ; need to reset on keypress 
...
;; render multiple balls tigether instead of only one
(check-expect (render-balls empty) MTS)
(check-expect (render-balls (cons B1 (cons B2 empty)))
              (place-ball B1 (place-ball B2 MTS)))
...
;; produce all balls in the list
(check-expect (next-balls empty) empty)
(check-expect (next-balls LOB1) LOB1)
(@template ListOfBall)
...
;; placea ball on MTS
(check-expect (place-ball B1 MTS)
              (place-image BALL (ball-x B1) (ball-y B1) MTS))

(check-expect (place-ball B2 MTS)
              (place-image BALL (ball-x B2) (ball-y B2) MTS))
#;
...
;; clear screen of balls
(check-expect (handle-key (cons B1 (cons B2 empty))
                          " ")
              empty)
(check-expect (handle-key (cons B1 (cons B2 empty))
                          "q")
              (cons B1 (cons B2 empty)))

;; (define (handle-key lob ke) empty)  ; -> stub
